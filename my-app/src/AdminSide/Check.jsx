    ...
    <div>
      ...
      <div className="container" >
        {checkProducts.length > 0 && checkProducts.map((item) => {
          return (<div  key={item.id}><Recent id={item.id} title={item.title} image={item.image} price={item.price} category={item.category} type={item.type} subTitle={item.subTitle} rate={item.rate} count={item.count} off={item.off} quantity={item.quantity} max_unit={item.max_unit} /></div>)
        })}
      </div>
    </div>
  )
